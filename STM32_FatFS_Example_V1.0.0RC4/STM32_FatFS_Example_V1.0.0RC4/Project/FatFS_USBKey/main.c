...
FIL  file;
FATFS USBH_fatfs;
char MessageW[] = "This is a FAT FS example on STM32 using USB Host interface.";
char MessageR[256];
...
int main(void)
{
  ...
  */  
  ...
  if (f_write(&file, MessageW, sizeof(MessageW), &ret_length) != FR_OK)
  {
  ...
  if (f_open(&file, "0:/STM32_File.txt", FA_READ) != FR_OK)
  {
    LCD_ErrLog("Cannot open 'STM32 File.txt' file\n");
  }
  else
  ...
  if (f_read(&file, MessageR, sizeof(MessageR), &ret_length) != FR_OK)
  {
    LCD_ErrLog("Cannot read from 'STM32 File.txt' file\n");
  }
  else
  {
    LCD_UsrLog("Text : \n"); 
